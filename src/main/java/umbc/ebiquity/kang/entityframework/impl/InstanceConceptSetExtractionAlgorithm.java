...
import umbc.ebiquity.kang.textprocessing.featureextractor.impl.LongestCommonPhraseAnalyzer;
...
//		}
		...
		List<EntityNode> relationNodes = new ArrayList<EntityNode>();
		List<EntityNode> instanceNodes = new ArrayList<EntityNode>();
	...
	private void extractConcepts(Map<Concept, Concept> conceptSet, List<EntityNode> instanceNodes, double distanceDegree){
		
		List<Feature> features = new ArrayList<Feature>(instanceNodes.size());
		for (EntityNode entity : instanceNodes) {
			features.add(entity);
		}
		
		Map<String, Double> commonInstancePhraseMap = commonPhraseAnalyzer.computeCommonPhrases(features);
