...
//	private String tripleStoreName;
	...
	@Override
	public void save(IInstanceDescriptionModel model, String repositoryName) {
		...
		metaDataRecord.put(RepositorySchemaConfiguration.TRIPLE_STORE_NUMBER_OF_TRIPLES, String.valueOf(numberOfTriples));
		...
		for (InstanceTripleSet instanceTripleSet : model.getInstanceTripleSets()) {
			...
			for (String relationLabel : relation2ValueMap.keySet()) {
				...
				for (String valueLabel : relation2ValueMap.get(relationLabel)) {
					tripleRecord.put(RepositorySchemaConfiguration.TRIPLE_RECORD_TYPE,
							RepositorySchemaConfiguration.TRIPLE_RECORD_TYPE_RELATION_VALUE);
					tripleRecord.put(RepositorySchemaConfiguration.TRIPLE_SUBJECT, subjectLabel);
					tripleRecord.put(RepositorySchemaConfiguration.TRIPLE_PREDICATE, relationLabel);
					tripleRecord.put(RepositorySchemaConfiguration.TRIPLE_OBJECT, valueLabel);
					...
					tripleRecord.put(RepositorySchemaConfiguration.NORMALIZED_TRIPLE_OBJECT, "");
					tripleRecord.put(RepositorySchemaConfiguration.NORMALIZED_TRIPLE_SUBJECT, "");
					tripleRecord.put(RepositorySchemaConfiguration.TRIPLE_PREDICATE_TYPE, predicateTypeStr);
			...
			for (String relationLabel : instance2ConceptSetMap.keySet()) {
				...
				for (Concept concept : instance2ConceptSetMap.get(relationLabel)) {
					...
					tripleRecord.put(RepositorySchemaConfiguration.TRIPLE_SUBJECT, subjectLabel);
					tripleRecord.put(RepositorySchemaConfiguration.TRIPLE_PREDICATE, relationLabel);
					tripleRecord.put(RepositorySchemaConfiguration.TRIPLE_OBJECT, concept.getConceptName());
					tripleRecord.put(RepositorySchemaConfiguration.TRIPLE_OBJECT_AS_CONCEPT_SCORE, String.valueOf(concept.getScore()));
					...
					tripleRecord.put(RepositorySchemaConfiguration.IS_FROM_INSTANCE, isFromInstance);
					addRecord(triplesStringBuilder, tripleRecord);
					numberOfTriples++;
				}
			}
		...
		for (Triple triple : model.getRelationTypeTriple()) {
			...
			tripleRecord.put(RepositorySchemaConfiguration.TRIPLE_SUBJECT, triple.getSubject());
			tripleRecord.put(RepositorySchemaConfiguration.TRIPLE_PREDICATE, triple.getPredicate());
			tripleRecord.put(RepositorySchemaConfiguration.TRIPLE_OBJECT, triple.getObject());
		...
		if (hasTriples) {
			...
			this.save(fileFullName, triplesStringBuilder.toString());
		}
	}
	...
	private void loadTriple(String line) throws MalformedURLException { 
		...
		String triple_record_type = (String) record.get(RepositorySchemaConfiguration.TRIPLE_RECORD_TYPE);
		if (triple_record_type.equals(RepositorySchemaConfiguration.TRIPLE_RECORD_TYPE_META_DATA)) {
	...
	private void getMetaData(JSONObject record) throws MalformedURLException { 
		String triple_store_URI = (String) record.get(RepositorySchemaConfiguration.TRIPLE_STORE_URI);
		String triple_store_Relations = (String) record.get(RepositorySchemaConfiguration.TRIPLE_STORE_NUMBER_OF_RELATIONS);
		String triple_store_Triples = (String) record.get(RepositorySchemaConfiguration.TRIPLE_STORE_NUMBER_OF_TRIPLES);
	...
	private void createTriple(JSONObject record) {
		...
		Triple triple = null;
		String triple_record_type = (String) record.get(RepositorySchemaConfiguration.TRIPLE_RECORD_TYPE);
		String subject = (String) record.get(RepositorySchemaConfiguration.TRIPLE_SUBJECT);
		String object = (String) record.get(RepositorySchemaConfiguration.TRIPLE_OBJECT);
		String predicate = (String) record.get(RepositorySchemaConfiguration.TRIPLE_PREDICATE);
		String n_object = (String) record.get(RepositorySchemaConfiguration.NORMALIZED_TRIPLE_OBJECT);
		String n_subject = (String) record.get(RepositorySchemaConfiguration.NORMALIZED_TRIPLE_SUBJECT);
		String predicate_Type = (String) record.get(RepositorySchemaConfiguration.TRIPLE_PREDICATE_TYPE);
		
		if (triple_record_type.equals(RepositorySchemaConfiguration.TRIPLE_RECORD_TYPE_CONCEPT_OF_INSTANCE)) { 
			boolean isFromInstance = Boolean.valueOf((String) record.get(RepositorySchemaConfiguration.IS_FROM_INSTANCE));
			// TODO chech the nullpointer exception
			double score = Double.valueOf((String) record.get(RepositorySchemaConfiguration.TRIPLE_OBJECT_AS_CONCEPT_SCORE));
			Concept concept = new Concept(object, isFromInstance);
			concept.setScore(score);
			triple = new Triple(subject, concept);
		} else if (triple_record_type.equals(RepositorySchemaConfiguration.TRIPLE_RECORD_TYPE_RELATION_VALUE)) {
			triple = new Triple(subject, n_subject, predicate, object, n_object);
		} else if (triple_record_type.equals(RepositorySchemaConfiguration.TRIPLE_RECORD_TYPE_RELATION_DEFINITION)) {
